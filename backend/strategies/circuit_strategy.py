...
#         "SPANDANA-EQ", "AGSTRA-EQ", "NEWGEN-EQ", "SUZLON-EQ", "RAMANEWS-BE", "SWSOLAR-EQ",
#         "PCJEWELLER-EQ", "BGRENERGY-EQ", "NAVKARURB-EQ", "SADHNANIQ-BE", "BCG-EQ", "RELINFRA-EQ",
#         "INDOTECH-EQ", "GOACARBON-EQ", "DISHTV-BE", "VERTOZ-BE", "VAKRANGEE-EQ", "KPIGREEN-EQ"
#     ],
#     "Nifty 100": [
#         "RELIANCE-EQ", "HDFCBANK-EQ", "INFY-EQ", "ICICIBANK-EQ", "TCS-EQ", "KOTAKBANK-EQ",
...
#                 if lp > 100 and is_at_lower_circuit(lp, lc):
...
circuit_stock_lists = {
    "Nifty 50": [
        ...
        "SPANDANA-EQ", "AGSTRA-EQ", "NEWGEN-EQ", "SUZLON-EQ", "RAMANEWS-BE", "SWSOLAR-EQ",
        "PCJEWELLER-EQ", "BGRENERGY-EQ", "NAVKARURB-EQ", "SADHNANIQ-BE", "BCG-EQ", "RELINFRA-EQ",
        "INDOTECH-EQ", "GOACARBON-EQ", "DISHTV-BE", "VERTOZ-BE", "VAKRANGEE-EQ", "KPIGREEN-EQ", "AYMSYNTEX-EQ", "SUYOG-BE",
        "CYBERTECH-BE"
    ],
    "Nifty 100": [
        "RELIANCE-EQ", "HDFCBANK-EQ", "INFY-EQ", "ICICIBANK-EQ", "TCS-EQ", "KOTAKBANK-EQ",
    ...
    while is_market_open():
        ...
        for stock, token in token_map.items():
            ...
            while retries > 0:
                try:
                    quote = api.get_quotes(exchange, token)
                    if quote and "lp" in quote and "lc" in quote:
                        break
                except Exception as e:
                    log_json("Quote Fetch Error", {"stock": stock, "error": str(e)})
                retries -= 1
            ...
            if not quote:
                log_json("Quote Fetch Error", {"stock": stock, "error": "Invalid/empty quote"})
                continue
            ...
            if is_at_lower_circuit(lp, lc):
                lc_all.append({"stock": stock, "ltp": lp})
                recent_lc_stocks[stock] = current_time

                if lp > 100:
                    filtered_stocks.append({"stock": stock, "ltp": lp})
        ...
        log_json("All LC Stocks", lc_all)
        log_json("Filtered LC Stocks", filtered_stocks)

        for entry in filtered_stocks:
            stock = entry["stock"]
            try:
                token = token_map[stock]
                quote = api.get_quotes(exchange, token)
                if not quote or "lp" not in quote or "sq1" not in quote:
                    continue
